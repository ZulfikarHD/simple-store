...
<template>
    ...
    <Head :title="`Keranjang Belanja - ${store.name}`">
    ...
    <div class="flex min-h-screen flex-col bg-background">
        ...
        <header
            class="ios-navbar fixed inset-x-0 top-0 z-50 border-b border-brand-blue-200/30 dark:border-brand-blue-800/30">
        ...
        <!-- Main Content -->
        <PullToRefresh>
            <main class="mx-auto w-full max-w-7xl flex-1 px-4 py-6 pb-56 sm:px-6 sm:py-8 sm:pb-8 lg:px-8">
                <!-- Back Button dengan iOS press feedback -->
                <Motion :initial="{ opacity: 0, x: -20 }" :animate="{ opacity: 1, x: 0 }"
                    :transition="springTransition">
                    <Motion :animate="{ scale: isBackPressed ? 0.95 : 1, opacity: isBackPressed ? 0.7 : 1 }"
                        ...
                        <Link :href="home()"
                            ...
                            @mousedown="isBackPressed = true" @mouseup="isBackPressed = false"
                            @mouseleave="isBackPressed = false" @touchstart.passive="isBackPressed = true"
                ...
                <Motion :initial="{ opacity: 0, y: 20 }" :animate="{ opacity: 1, y: 0 }"
                    :transition="{ ...springTransition, delay: 0.05 }" class="mb-6 sm:mb-8">
                ...
                <AnimatePresence>
                    <Motion v-if="isEmpty" :initial="{ opacity: 0, scale: 0.95 }" :animate="{ opacity: 1, scale: 1 }"
                        :exit="{ opacity: 0, scale: 0.95 }" :transition="{ ...springTransition, delay: 0.1 }">
                        <EmptyState icon="🛒" title="Keranjang Kosong"
                ...
                <div v-if="!isEmpty" class="grid gap-6 lg:grid-cols-3 lg:gap-8">
                    ...
                    <div class="lg:col-span-2">
                        <div class="space-y-3 sm:space-y-4">
                            <CartItem v-for="(item, index) in cart.items" :key="item.id" :item="item" :index="index" />
                        </div>
                    </div>

                    <!-- Order Summary - Desktop Only dengan slide animation -->
                    <Motion :initial="{ opacity: 0, x: 20 }" :animate="{ opacity: 1, x: 0 }"
                        :transition="{ ...springTransition, delay: 0.2 }" class="hidden lg:col-span-1 lg:block">
                        <div class="premium-card sticky top-24 rounded-2xl p-6">
                            ...
                            <div class="space-y-3">
                                <div class="flex items-center justify-between text-sm">
                                    ...
                                    <Motion :key="cart.subtotal" :initial="{ scale: 1.1, opacity: 0 }"
                                        :animate="{ scale: 1, opacity: 1 }" :transition="bouncyTransition"
                                ...
                                <div
                                    class="flex items-center justify-between rounded-xl bg-gradient-to-r from-brand-blue-50 to-brand-gold-50 p-3 dark:from-brand-blue-900/30 dark:to-brand-gold-900/20">
                            ...
                            <Motion :animate="{ scale: isCheckoutPressed ? 0.95 : 1 }" :transition="snappyTransition">
                                <Link :href="checkoutShow()" @click="handleCheckoutClick">
                                    <Button size="lg"
                                        ...
                                        @mousedown="isCheckoutPressed = true" @mouseup="isCheckoutPressed = false"
            ...
            <!-- Footer -->
            <footer
                class="mt-16 hidden border-t border-brand-blue-100 bg-gradient-to-b from-brand-blue-50/50 to-white dark:border-brand-blue-900/30 dark:from-brand-blue-900/20 dark:to-background md:block">
    ...
    <!-- Mobile Sticky Checkout Footer - Di luar PullToRefresh untuk fixed positioning yang benar -->
    <AnimatePresence>
        <Motion v-if="!isEmpty" :initial="{ y: 100, opacity: 0 }" :animate="{ y: 0, opacity: 1 }"
            :exit="{ y: 100, opacity: 0 }" :transition="springTransition"
            class="ios-glass fixed inset-x-0 bottom-20 z-40 border-t border-brand-blue-200/30 p-4 dark:border-brand-blue-800/30 md:bottom-0 lg:hidden">
