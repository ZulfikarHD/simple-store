...
const isStickyWhatsAppPressed = ref(false)
...
<template>
    ...
    <div class="min-h-screen bg-background">
        ...
        <main class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
            <!-- Important: WhatsApp Confirmation Alert -->
            <Motion
                :initial="{ opacity: 0, y: -10, scale: 0.95 }"
                :animate="{ opacity: 1, y: 0, scale: 1 }"
                :transition="{ ...bouncyTransition, delay: 0.1 }"
                class="mb-6 overflow-hidden rounded-2xl border-2 border-green-500 bg-gradient-to-r from-green-50 to-green-100 p-4 shadow-lg shadow-green-500/10 dark:border-green-600 dark:from-green-900/30 dark:to-green-800/30"
            >
                <div class="flex items-start gap-3">
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-green-500 shadow-md shadow-green-500/30">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="mt-1 text-sm text-green-700 dark:text-green-400">
                            Pesanan Anda <strong>belum dikonfirmasi</strong>. Silakan kirim pesan WhatsApp ke toko untuk mengkonfirmasi pesanan.
                        </p>
                    </div>
                </div>
            </Motion>
        ...
        <!-- Sticky WhatsApp CTA untuk Mobile -->
        <Motion
            :initial="{ opacity: 0, y: 20 }"
            :animate="{ opacity: 1, y: 0 }"
            :transition="{ ...springTransition, delay: 0.5 }"
            ...
            <div class="mx-4 overflow-hidden rounded-2xl border border-green-200 bg-white/95 p-3 shadow-xl shadow-green-500/10 backdrop-blur-md dark:border-green-800/30 dark:bg-background/95">
                <div class="flex items-center gap-3">
                    <div class="min-w-0 flex-1">
                        <p class="truncate text-xs font-medium text-muted-foreground">Konfirmasi pesanan Anda</p>
                        <p class="truncate text-sm font-bold text-foreground">{{ order.order_number }}</p>
                    </div>
                    <Motion
                        :animate="{ scale: isStickyWhatsAppPressed ? 0.95 : 1 }"
                        :transition="snappyTransition"
                    >
                        <Button
                            size="default"
                            class="ios-button shrink-0 gap-2 rounded-xl bg-green-600 px-4 shadow-lg shadow-green-600/30 hover:bg-green-700"
                            @click="openWhatsApp"
                            @mousedown="isStickyWhatsAppPressed = true"
                            @mouseup="isStickyWhatsAppPressed = false"
                            @mouseleave="isStickyWhatsAppPressed = false"
                            @touchstart.passive="isStickyWhatsAppPressed = true"
                            @touchend="isStickyWhatsAppPressed = false"
                        >
                            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
                                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                            </svg>
